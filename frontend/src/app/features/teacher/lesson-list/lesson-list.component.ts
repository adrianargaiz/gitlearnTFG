            ...
            <table class="gl-table">
              ...
              <tbody>
                @for (lesson of filtered(); track lesson._id) {
                  <tr>
                    <td style="font-family:monospace;font-size:12px">{{ lesson.orden }}</td>
                    <td>
                      <div style="font-weight:700;color:var(--text);font-size:14px">{{ lesson.titulo }}</div>
                      @if (lesson.descripcion) {
                        <div style="font-size:12px;color:var(--muted);margin-top:2px;max-width:240px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">{{ lesson.descripcion }}</div>
                      }
                    </td>
                    <td>
                      <span class="gl-badge"
                    ...
                    <td>{{ lesson.ejerciciosCount ?? '—' }}</td>
                    <td style="font-size:12px">{{ formatDate(lesson.updatedAt) }}</td>
                  ...
                  <tr>
                    <td colspan="8" style="text-align:center;padding:64px 16px">
